...
Assertion.prototype = {
  ...
  throw: function(error, message){
    ...
    if ((message === undefined) && !(error instanceof Function)) {
      message = error;
      error = undefined;
    }

    if ((error !== undefined) && !(error instanceof Function)) {
      throw new Error('error is expected to be a Function, but got ' + error.toString());
    }
    ...
    errorInfo = 'expected an exception to be thrown';

    if (ok) {
      if ((error) && (err.name !== error.name)) {
        ok = false;
        errorInfo = errorInfo + " as an instance of '" + error.name + 
        "', but got '" + err.name + "'";
      } else if (message) {
        if ('string' == typeof message) {
          ok = message === err.message;
           errorInfo = errorInfo + " with a message matching '" 
            + message + "', but got '" + err.message + "'";
